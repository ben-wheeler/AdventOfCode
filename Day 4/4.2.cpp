...
std::vector<Board> checkWin(std::vector<Board> inp, int num)
{
    for (size_t i = 0; i < inp.size(); i++)
    {
        bool current = false;
...
int main()
{
    ...
    for (int i = 0; i < number_call_outs.size(); i++)
    {
        ...
        boards = checkWin(boards,number_call_outs[i]);
    }
